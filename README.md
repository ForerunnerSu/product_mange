...
│  │                                   │                          │  │
│  │  ┌────────────────┐    ┌──────────┴──────────┐              │  │
│  │  │  Service Worker │    │    Quagga2 库        │              │  │
│  │  │  (sw.js v5)     │    │   (条码识别)          │              │  │
│  │  │                  │    │  ·摄像头调用          │              │  │
│  │  │ ·离线缓存        │    │  ·EAN/UPC/Code128    │              │  │
│  │  │ ·导航优先网络    │    │  ·自动填入表单        │              │  │
│  │  │ ·自动检测更新    │    └──────────────────────┘              │  │
│  │  └────────────────┘                                           │  │
│  └───────────────────────────────────────────────────────────────┘  │
...
│                          │                       │                │
│                          ▼                       ▼                │
│                   GitHub Pages          product-mange.pages.dev  │
│               (备用访问地址)             (主访问地址)              │
└─────────────────────────────────────────────────────────────────┘
...
│      │                                                            │
│      ├─→ 页面渲染 (HTML + CSS + Font Awesome 图标)                │
│      │                                                            │
│      ├─→ 用户录入商品                                              │
│      │       ├─ 扫码? → Quagga2 → 摄像头 → 识别条码               │
│      │       │         → 查商品库(localStorage) → 自动填表单      │
│      │       └─ 手动? → 用户填写表单                              │
│      │                    │                                       │
│      │                    ▼                                       │
│      │       保存到 localStorage (本地即时显示)                    │
│      │                    │                                       │
│      │                    ▼                                       │
│      │       Firebase SDK 上传到云端 (带重试机制)                  │
│      │                    │                                       │
│      │                    ▼                                       │
│      │       Firebase 触发 on('value') → 推送到所有在线设备        │
│      │                                                            │
│      ├─→ 用户删除商品                                              │
│      │       ├─ 本地删除 + 标记墓碑(localStorage)                  │
│      │       ├─ 云端删除 products/{id}                             │
│      │       └─ 云端写入 deleted/{id} (墓碑)                      │
│      │            │                                               │
│      │            ▼                                               │
...
```
    ...
    └───────────────┘              └───────┬───────┘
            ...
            └───────┬───────┘      └───────┬───────┘      └───────┬───────┘
                    ...
                    ▼                      ▼                      ▼
            ┌───────────────┐      ┌───────────────┐      ┌───────────────┐
            │  更新本地数据  │      │  更新本地数据  │      │  更新本地数据  │
            │  刷新页面UI   │      │  刷新页面UI   │      │  刷新页面UI   │
            └───────────────┘      └───────────────┘      └───────────────┘
```
